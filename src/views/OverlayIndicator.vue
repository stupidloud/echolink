...
import { currentLocale, currentScript } from '../i18n'
...
async function handleTranscribeWebM(settings) {
  try {
    ...
    const text = await invoke(cmd, { audioB64: base64, settings, language: currentLocale(), script: currentScript() })
...
async function handleTranscribePcm(settings) {
  try {
    ...
    const text = await invoke('transcribe_audio_sse', { audioB64: base64, settings, language: currentLocale(), script: currentScript() })
